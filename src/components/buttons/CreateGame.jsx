...
function CreateGameButton (props) {
  ...
  async function createGame () {
      ...
      for (const question of newQuestions) {
        ...
        for (const option of question.answers) {
          ...
          if (option.optionField === '' && option.optionCorrect === true) {
            failNotify(
              'Please enter your option after select it as true answer'
            );
            return;
          }
    ...
    const responseForCreatingGame = await fetchPOST(
      'admin/quiz/new',
      { name: newGame },
      'newGame'
    );
    if (responseForCreatingGame.status === 200) {
      ...
      const responseForUpdatingGame = await fetchPut(
        'admin/quiz/' + quizId,
        NewQuiz,
        'token'
      );
  ...
  return (
    <>
      ...
      <Dialog open={open} onClose={closeWindow}>
        ...
        <DialogActions>
          <Button name="more-info-btn" onClick={moreInfo}>
            More
          </Button>
          ...
          <Button name="cancel-create-btn" onClick={closeWindow}>
            Cancel
          </Button>
          <Button name="submit-new-create-btn" onClick={createGame}>
            Create
          </Button>
        </DialogActions>
      </Dialog>
