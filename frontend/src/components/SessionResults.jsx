...
export default function SessionResult (props) {
  ...
  useEffect(async () => {
    const sessionResults = (
      await fetchGET(`admin/session/${sessionId}/results`, 'token')
    ).results;
  ...
  useEffect(async () => {
    const sessionQuestions = (
      await fetchGET(`admin/session/${sessionId}/status`, 'token')
    ).results.questions;
    ...
    for (let j = 0; j < sessionQuestion.length; j++) {
      if (sessionResult[i].answers[j].correct) {
        ...
        const questionStartedAt = new Date(
          sessionResult[i].answers[j].questionStartedAt
        );
        scores =
          scores +
          (sessionQuestion[j].points *
            (answeredAt.getTime() - questionStartedAt.getTime())) /
            1000;
      }
    }
    newResultInfo.push({
      name: sessionResult[i].name,
      point: points,
      score: scores.toFixed(1),
      correctNum: correctNums,
    });
  }
  ...
  for (let i = 0; i < sessionQuestion.length; i++) {
    ...
    for (let j = 0; j < sessionResult.length; j++) {
      if (sessionResult[j].answers[i].answeredAt !== null) {
        ...
        const questionStartedAt = new Date(
          sessionResult[j].answers[i].questionStartedAt
        );
        timecost =
          timecost +
          (answeredAt.getTime() - questionStartedAt.getTime()) / 1000;
      }
    ...
    correctPercentage.push({
      question: `Q${i + 1}`,
      percentage: ((people / sessionResult.length) * 100).toFixed(1),
    });
    if (timecost === 0 || peopleAnswered === 0) {
      ...
      averageTime.push({
        questionTime: `Q${i + 1}`,
        AverageTimeCost: (timecost / peopleAnswered).toFixed(1),
      });
    }
  }
